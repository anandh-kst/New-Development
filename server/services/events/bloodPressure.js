...
export const saveBloodPressureEvent = async (webhookData) => {
  try {
    ...
    for (const event of bpEvents) {
      ...
      const bpMetricsMap = [
        { type: METRIC_TYPE.BLOOD_PRESSURE_SYSTOLIC, value: bpData.systolic_mmHg_int, unit: "mmHg" },
        { type: METRIC_TYPE.BLOOD_PRESSURE_DIASTOLIC, value: bpData.diastolic_mmHg_int, unit: "mmHg" },
      ];
    ...
    if (metricsToSave.length > 0) {
      ...
      console.log(`Saved ${metricsToSave.length} blood_pressure_event metrics for ${user_id}`);
    } else {
      console.log(`No valid blood_pressure_event metrics to save for ${user_id}`);
    }
