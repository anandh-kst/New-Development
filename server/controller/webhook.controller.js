    ...
    try {
      ...
      if (!service)
        return res
          .status(200)
          .json({
            error: `No service implemented for ${payload.data_structure}`,
          });
      ...
      return res
        .status(200)
        .json({ message: `${payload.data_structure} processed successfully` });
    } catch (err) {
      console.error(
        `[Webhook] Error processing ${req.body.data_structure}:`,
        err
      );
